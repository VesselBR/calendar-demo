...
const BookingModal = (props :BookingModalProps) => {
	...
	let openButton
	openButton = <PinModal shopId={props.shopId} setParent={(id: number)=>setAuditId(id)} buttonTitle="Abrir Comanda" ></PinModal>
	if (props.agendaAudit === false){
		openButton = <Button color="success" onClick={() => { onPutEvent() }}> Abrir Comanda </Button>
	} else {
		if(auditId){
			openButton = <Button 
				color="success" 
	...
	let putButton
	putButton = <PinModal shopId={props.shopId} setParent={(id: number)=>setAuditId(id)} buttonTitle="Atualizar" ></PinModal>
	if (props.agendaAudit === false){
		putButton = <Button color="success" onClick={() => { onPutEvent() }}> Atualizar (sem pin) </Button>
	} else {
		if(auditId){
			putButton = <Button 
				color="success" 
	...
	let delButton
	delButton = <PinModal shopId={props.shopId} setParent={(id: number)=>setAuditId(id)} buttonTitle="Excluir" ></PinModal>
	if (props.agendaAudit === false){
		delButton = <Button color="error" onClick={() => { onDelEvent() }}> Cancelar (sem pin) </Button>
	} else {
		if(auditId){
			delButton = <Button 
				color="error" 
	...
	if (booking?.comanda_id){
		comandaButton = <Button
			variant="contained"
			style={{ float: 'right' }}
			onClick={onViewComanda} >Consultar Comanda
		</Button>
	}	else {
				...
				<Form.Group>
						...
						<Form.Select 
							...
							onChange={e => {
								...
								setBooking({...booking!, customer_id: parseInt(e.target.value) })
							  }	
							}
						>
						{
									customers.map((customer) => {
										return (<option
											key={customer.value}
											value={customer.value} >{customer.label}</option>)
									})
						}							
        				</Form.Select>												
				</Form.Group>	
					<Form.Group>
						...
						>
						{
									props.staffs.map((staff) => {
										return (<option
											key={staff.value}
											value={staff.value} >{staff.label}</option>)
									})
						}							
        				</Form.Select>												
					</Form.Group>
